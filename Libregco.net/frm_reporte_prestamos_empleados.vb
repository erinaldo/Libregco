...
Public Class frm_reporte_prestamos_empleados
    ...
    Private Sub frm_reporte_prestamos_empleados_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Me.CenterToParent()
        ...
        Try
            ...
            If chkEspecificarVenc.Checked = False Then
                If CDate(dtpIngresoInicial.Value) > CDate(dtpIngresoFinal.Value) Then
                    MessageBox.Show("La fecha de inicial es mayor a la fecha final" & vbNewLine & vbNewLine & "Por favor revisar la fecha de ingreso para procesar el reporte.", "Rango de Fechas no válidos", MessageBoxButtons.OK, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1)
                    Exit Sub
                End If
            End If
    ...
    Private Sub txtIDTipoPrestamo_Leave(sender As Object, e As EventArgs) Handles txtIDTipoPrestamo.Leave
        ConLibregco.Open()
        cmd = New MySqlCommand("Select Descripcion from TipoPrestamoEmp Where IDTipoPrestamoEmp='" + txtIDTipoPrestamo.Text + "'", ConLibregco)
        txtTipoPrestamo.Text = Convert.ToString(cmd.ExecuteScalar())
        ConLibregco.Close()
        If txtTipoPrestamo.Text = "" Then txtIDTipoPrestamo.Clear()
    End Sub
End Class