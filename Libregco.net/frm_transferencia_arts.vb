...
Imports System.IO
Public Class frm_transferencia_arts

    Dim sqlQ As String=""
    Dim cmd As MySqlCommand
    Dim Ds, Ds1, Ds2 As New DataSet
    Dim Adaptador As MySqlDataAdapter
    Friend IDArticulo, lblIDUsuario, lblIDAlmacenEntrada, lblIDAlmacenSalida, lblIDPrecio, lblIDMedida, lblNulo, lblCheckDuplicates, lblIDAlmacen As New Label
    Dim Permisos As New ArrayList

    Private Sub frm_transferencia_arts_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Me.CenterToParent()
    ...
    Private Sub FillAlmacenRealizacion()
    ...
    Private Sub FillAlmacen()
        Dim DatasetEntrada, DatasetSalida As New DataSet
        DatasetSalida.Clear()
        Con.Open()
        ...
        Adaptador = New MySqlDataAdapter(cmd)
        Adaptador.Fill(DatasetSalida, "Almacen")
        CbxAlmacenSalida.Items.Clear()
        Dim TablaSalida As DataTable = DatasetSalida.Tables("Almacen")

        For Each Fila As DataRow In TablaSalida.Rows
            CbxAlmacenSalida.Items.Add(Fila.Item("Almacen"))
        Next

        DatasetEntrada.Clear()
        cmd = New MySqlCommand("SELECT Almacen FROM Almacen Where Desactivar=0", Con)
        Adaptador = New MySqlDataAdapter(cmd)
        Adaptador.Fill(DatasetEntrada, "Almacen")
        cbxAlmacenEntrada.Items.Clear()
        Dim TablaEntrada As DataTable = DatasetEntrada.Tables("Almacen")

        For Each Fila As DataRow In TablaEntrada.Rows
            cbxAlmacenEntrada.Items.Add(Fila.Item("Almacen"))
        Next
    ...
    Private Sub ColumnasDgvArticulos()
        ...
        With DgvArticulos
            .Columns.Add("IDTransfArtDetalle", "IDTransfArtDetalle")  '0
            .Columns.Add("IDTransfArt", "IDTransfArt")    '1
    ...
    Private Sub CbxAlmacenSalida_SelectedIndexChanged(sender As Object, e As EventArgs) Handles CbxAlmacenSalida.SelectedIndexChanged
        SetIDAlmacenSalida()
        ImpedirMismoAlmacen()
        HabilitarGuardar()
    End Sub

    Private Sub SetIDAlmacenSalida()
        Con.Open()
        cmd = New MySqlCommand("SELECT IDAlmacen FROM Almacen WHERE Almacen= '" + CbxAlmacenSalida.SelectedItem + "'", Con)
        lblIDAlmacenSalida.Text = Convert.ToString(cmd.ExecuteScalar())
        Con.Close()
    End Sub

    Private Sub cbxAlmacenEntrada_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cbxAlmacenEntrada.SelectedIndexChanged
        SetIDAlmacenEntrada()
        ImpedirMismoAlmacen()
        HabilitarGuardar()
    End Sub

    Private Sub SetIDAlmacenEntrada()
        Con.Open()
        cmd = New MySqlCommand("SELECT IDAlmacen FROM Almacen WHERE Almacen= '" + cbxAlmacenEntrada.SelectedItem + "'", Con)
        lblIDAlmacenEntrada.Text = Convert.ToString(cmd.ExecuteScalar())
        Con.Close()
    End Sub

    Private Sub ImpedirMismoAlmacen()
        If CbxAlmacenSalida.Text = cbxAlmacenEntrada.Text Then
            MessageBox.Show("No se puede realizar una transferencia de inventario hacia el mismo almacén.", "Operación no válida", MessageBoxButtons.OK, MessageBoxIcon.Exclamation, MessageBoxDefaultButton.Button1)
            cbxAlmacenEntrada.Items.Clear()
            CbxAlmacenSalida.Items.Clear()
            FillAlmacen()
        End If
    ...
    Sub SetCosto()
        Try
            Ds.Clear()
            ConLibregco.Open()
            cmd = New MySqlCommand("SELECT IDPrecios,Costo FROM PrecioArticulo WHERE IDArticulo= '" + txtIDArticulo.Text + "' AND IDMedida='" + lblIDMedida.Text + "' and PrecioArticulo.Nulo=0", ConLibregco)
            Adaptador = New MySqlDataAdapter(cmd)
            Adaptador.Fill(Ds, "PrecioArticulo")
            ConLibregco.Close()
    ...
    Private Sub HabilitarControles()
        cbxAlmacenEntrada.Enabled = True
        CbxAlmacenSalida.Enabled = True
    ...
    Private Sub DesHabilitarControles()
        cbxAlmacenEntrada.Enabled = False
        CbxAlmacenSalida.Enabled = False
    ...
    Private Sub btnAplicarMonto_Click(sender As Object, e As EventArgs) Handles btnAplicarMonto.Click
        Try
            ...
            Else
                ...
                With DgvArticulos
                    .Rows.Add("", "", lblIDPrecio.Text, IDArticulo.Text, txtDescripcionArticulo.Text, lblIDMedida.Text, CbxMedida.Text, txtCantidadArticulo.Text, CDbl(txtPrecio.Text), CDbl((txtPrecio.Text) * CDbl(txtCantidadArticulo.Text)))
                End With
...
Final:
    ...
    Private Sub HabilitarGuardar()
        Dim Neto
        Neto = CDbl(txtNeto.Text)

        If cbxAlmacenEntrada.Text = "" Or CbxAlmacenSalida.Text = "" Or Neto = CDbl(0) Then
    ...
    Private Sub EliminarArticulo()
        Dim IDTransferenciaArtDetalle As New Label
        IDTransferenciaArtDetalle.Text = DgvArticulos.CurrentRow.Cells(0).Value

        If IDTransferenciaArtDetalle.Text = "" Or txtIDTransferenciaArt.Text = "" Then
            Exit Sub
        Else
            sqlQ = "Delete from transferenciaartdetalle Where IDTransferenciaArtDetalle = (" + IDTransferenciaArtDetalle.Text + ")"
            GuardarDatos()
            CalcExistencia()
            CalcExistenciaAlm()

            MessageBox.Show("Artículo eliminado satisfactoriamente de la transferencia de inventario.", "Eliminado Exitoso", MessageBoxButtons.OK, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1)
        End If
    ...
    Private Sub ActualizarCambios()
        'Trabajar aqui cuando se puedan guardar
    End Sub
    ...
    Private Sub UltTransferencia()
        Try

            If txtIDTransferenciaArt.Text = "" Then
                Con.Open()
                cmd = New MySqlCommand("Select IDTransferenciaArt from transfenciaarticulos where IDTransferenciaArt= (Select Max(IDTransferenciaArt) from transfenciaarticulos)", Con)
                txtIDTransferenciaArt.Text = Convert.ToDouble(cmd.ExecuteScalar())
                Con.Close()
            End If
    ...
    Private Sub ImprimirDocumento()
        ...
        Else
            Dim Result As MsgBoxResult = MessageBox.Show("Desea imprimir la transferencia de inventarios?", "Imprimir Transferencia", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1)
    ...
    Private Sub InsertArticulos()
        Dim IDTransferenciaArtDetalles, IDTransferenciaArt, IDPrecio, IDArticulo, Descripcion, IDMedida, Medida, Cantidad, Precio, Importe As New Label
        Dim x As Integer = 0
...
Inicio:
        ...
        IDTransferenciaArtDetalles.Text = DgvArticulos.Rows(x).Cells(0).Value
        IDTransferenciaArt.Text = DgvArticulos.Rows(x).Cells(1).Value
        ...
        If IDTransferenciaArtDetalles.Text = "" Then
            sqlQ = "INSERT INTO transferenciaartdetalle (IDTransferenciaArticulo,IDPrecio,IDArticulo,Descripcion,IDMedida,Medida,Cantidad,Precio,Importe) VALUES ('" + txtIDTransferenciaArt.Text + "','" + IDPrecio.Text + "','" + IDArticulo.Text + "','" + Descripcion.Text + "','" + IDMedida.Text + "','" + Medida.Text + "','" + Cantidad.Text + "','" + Precio.Text + "','" + Importe.Text + "')"
            GuardarDatos()
            x = x + 1
            GoTo Inicio
        Else
...
Fin:
    ...
    Sub RefrescarTablaArticulos()
        ...
        Try
            ...
            Else
                DgvArticulos.Rows.Clear()
                Con.Open()
                Dim Consulta As New MySqlCommand("Select * FROM transferenciaartdetalle Where IDTransferenciaArticulo='" + txtIDTransferenciaArt.Text + "'", Con)
    ...
    Private Sub txtNeto_TextChanged(sender As Object, e As EventArgs) Handles txtNeto.TextChanged
        HabilitarGuardar()
    End Sub
    ...
    Private Sub btnBlanquear_Click(sender As Object, e As EventArgs) Handles btnBlanquear.Click
        Try
            ...
            If txtIDTransferenciaArt.Text = "" And Counter >= 1 Then
            ...
            If txtIDTransferenciaArt.Text > 0 And Counter >= 1 Then
    ...
    Private Sub SetSecondID()
        Try
            ...
            Con.Open()
            DsTemp.Clear()
            cmd = New MySqlCommand("SELECT * FROM TipoDocumento WHERE IDTipoDocumento=11", Con)
            Adaptador = New MySqlDataAdapter(cmd)
            ...
            sqlQ = "UPDATE Transfenciaarticulos SET SecondID='" + lblSecondID.Text + "' WHERE IDTransferenciaArt='" + txtIDTransferenciaArt.Text + "'"
            GuardarDatos()

            sqlQ = "UPDATE TipoDocumento SET UltimaSecuencia='" + UltSecuencia.Text + "' WHERE IDTipoDocumento=11"
            GuardarDatos()
    ...
    Private Sub btnGuardar_Click(sender As Object, e As EventArgs) Handles btnGuardar.Click
        If txtIDTransferenciaArt.Text = "" Then 'Si no hay ajuste
            If Permisos(1) = 0 Then
                MessageBox.Show("Usted no tiene los privilegios y/o permisos suficientes para guardar nuevos registros en la base de datos.", "No tiene los permisos necesarios", MessageBoxButtons.OK, MessageBoxIcon.Information)
                Exit Sub
            End If
            Dim Result As MsgBoxResult = MessageBox.Show("Está seguro que desea guardar la guardar la transferencia de inventario en la base de datos?", "Guardar Transferencia de Inventario", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1)
            If Result = MsgBoxResult.Yes Then
                ConvertDouble()
                sqlQ = "INSERT INTO Transfenciaarticulos (IDTipoDocumento,Fecha,Hora,IDUsuario,Referencia,Comentario,IDAlmacenSalida,IDAlmacenEntrada,Total,Nulo,Impreso) VALUES (11,'" + txtFecha.Text + "','" + txtHora.Text + "','" + lblIDUsuario.Text + "','" + txtReferencia.Text + "','" + txtComentario.Text + "','" + lblIDAlmacenSalida.Text + "','" + lblIDAlmacenEntrada.Text + "','" + txtNeto.Text + "','" + lblNulo.Text + "',0)"
                GuardarDatos()
                ConvertCurrent()
                UltTransferencia()
                SetSecondID()
        ...
        Else
            If Permisos(2) = 0 Then
                ...
                Exit Sub
            End If
            Dim Result As MsgBoxResult = MessageBox.Show("Está seguro que desea guardar los cambios realizados en la transferencia de inventario?", "Guardar Cambios", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1)
            If Result = MsgBoxResult.Yes Then
                ConvertDouble()
                sqlQ = "UPDATE Transfenciaarticulos SET IDUsuario='" + lblIDUsuario.Text + "',Referencia='" + txtReferencia.Text + "',Comentario='" + txtComentario.Text + "',IDAlmacenSalida='" + lblIDAlmacenSalida.Text + "',IDAlmacenEntrada='" + lblIDAlmacenEntrada.Text + "',Total='" + txtNeto.Text + "',Nulo='" + lblNulo.Text + "' WHERE IDTransferenciaArt= (" + txtIDTransferenciaArt.Text + ")"
                GuardarDatos()
                ConvertCurrent()
                InsertArticulos()
                CalcExistencias()
    ...
    Private Sub btnBuscar_Click(sender As Object, e As EventArgs) Handles btnBuscar.Click
        frm_buscar_transferencia_art.ShowDialog(Me)
    End Sub
    ...
    Private Sub btnAnular_Click(sender As Object, e As EventArgs) Handles btnAnular.Click
        ...
        End If
        If ChkNulo.Checked = True Then
            Dim Result1 As MsgBoxResult = MessageBox.Show("El registro de transferencia de inventario No. " & txtIDTransferenciaArt.Text & " ya está anulada en el sistema. Desea activarla?", "Recuperar Registro de Movimientos de Inventario", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1)
            If Result1 = MsgBoxResult.Yes Then
                ...
                ChkNulo.Checked = False
                ConvertDouble()
                sqlQ = "UPDATE Transfenciaarticulos SET IDUsuario='" + lblIDUsuario.Text + "',IDSucursal='" + DtEmpleado.Rows(0).item("IDSucursal").ToString() + "',IDAlmacen='" + lblIDAlmacen.Text + "',Referencia='" + txtReferencia.Text + "',Comentario='" + txtComentario.Text + "',IDAlmacenSalida='" + lblIDAlmacenSalida.Text + "',IDAlmacenEntrada='" + lblIDAlmacenEntrada.Text + "',Total='" + txtNeto.Text + "',Nulo='" + lblNulo.Text + "' WHERE IDTransferenciaArt= (" + txtIDTransferenciaArt.Text + ")"
                GuardarDatos()
                CalcExistencias()
                ConvertCurrent()
                MessageBox.Show("Los cambios se realizaron satisfactoriamente.", "Guardado Exitoso", MessageBoxButtons.OK, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1)
            End If
        ElseIf txtIDTransferenciaArt.Text = "" Then
            MessageBox.Show("No hay un registro de transferencia de inventario abierto para desactivar.", "No se encontró registro", MessageBoxButtons.OK, MessageBoxIcon.Exclamation, MessageBoxDefaultButton.Button1)
        Else
            Dim Result As MsgBoxResult = MessageBox.Show("Está seguro que desea anular el registro de transferencia de inventario No. " & txtIDTransferenciaArt.Text & " del sistema?", "Anular Transferencia de Inventario de Sistema", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1)
            If Result = MsgBoxResult.Yes Then
                frm_superclave.ShowDialog(Me)
                If ControlSuperClave = 1 Then
                    Exit Sub
    ...
    Private Sub frm_transferencia_arts_Resize(sender As Object, e As EventArgs) Handles MyBase.Resize
        PropiedadesDgvArticulos()
    End Sub
    ...
    Private Sub BlanquearListadoToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles BlanquearListadoToolStripMenuItem.Click
