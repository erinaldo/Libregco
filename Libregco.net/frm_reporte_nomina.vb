...
Public Class frm_reporte_nomina
    ...
    Private Sub frm_reporte_nomina_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Me.CenterToParent()
    ...
    Private Sub FillReportes()
        Try
            Ds.Clear()
            Con.Open()
            cmd = New MySqlCommand("SELECT Descripcion FROM Reportes INNER JOIN PaperSize on Reportes.IDPaperName=PaperSize.IDPaperSize Where MenuString='Nomina' UNION ALL SELECT Descripcion FROM Reportes INNER JOIN PaperSize on Reportes.IDPaperName=PaperSize.IDPaperSize Where MenuString='Nomina Listado'", Con)
    ...
    Private Sub txtIDTipoNomina_Leave(sender As Object, e As EventArgs) Handles txtIDTipoNomina.Leave
        ConLibregco.Open()
        cmd = New MySqlCommand("Select Descripcion from TipoNomina Where IDTipoNomina='" + txtIDTipoNomina.Text + "'", ConLibregco)
        txtTipoNomina.Text = Convert.ToString(cmd.ExecuteScalar())
        ConLibregco.Close()
        If txtTipoNomina.Text = "" Then txtIDTipoNomina.Clear()
    End Sub
    ...
    Private Sub txtIDNomina_Leave(sender As Object, e As EventArgs) Handles txtIDNomina.Leave
        Con.Open()
        cmd = New MySqlCommand("Select SecondID from Nomina Where IDNomina='" + txtIDNomina.Text + "'", Con)
        ...
        Try
            ...
            If chkEspecificarVenc.Checked = False Then
                If CDate(dtpIngresoInicial.Value) > CDate(dtpIngresoFinal.Value) Then
                    MessageBox.Show("La fecha inicial es mayor a la fecha final" & vbNewLine & vbNewLine & "Por favor revisar el rango de fechas para procesar el reporte.", "Rango de Fechas no válidos", MessageBoxButtons.OK, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1)
                    Exit Sub
                End If
            End If
            ...
            Else
                With crParameterRangeValue
                    .StartValue = dtpIngresoInicial.Value
                    .UpperBoundType = RangeBoundType.BoundInclusive
                    .EndValue = dtpIngresoFinal.Value
                    .LowerBoundType = RangeBoundType.BoundInclusive
                End With
            End If
    ...
    Private Sub btnBuscarTipoNomina_Click(sender As Object, e As EventArgs) Handles btnBuscarTipoNomina.Click
