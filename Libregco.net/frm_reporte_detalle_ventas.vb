...
Public Class frm_reporte_detalle_ventas
    ...
    Private Sub frm_reporte_detalle_ventas_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Me.CenterToParent()
    ...
    Private Sub FillReportes()
        Try
            Ds.Clear()
            Con.Open()
            cmd = New MySqlCommand("SELECT Descripcion FROM Reportes INNER JOIN PaperSize on Reportes.IDPaperName=PaperSize.IDPaperSize Where MenuString='Detalle Ventas' and Reportes.Activo=1 and PaperSize.Activo=1 ORDER BY NoOrden ASC", Con)
            Adaptador = New MySqlDataAdapter(cmd)
            Adaptador.Fill(Ds, "Reportes")
