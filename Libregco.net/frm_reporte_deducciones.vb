...
Public Class frm_reporte_deducciones
    ...
    Private Sub frm_reporte_deducciones_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Me.CenterToParent()
    ...
    Private Sub chkSinEspecNacimiento_CheckedChanged(sender As Object, e As EventArgs) Handles chkEspecificarVenc.CheckedChanged
        If chkEspecificarVenc.Checked = True Then
            dtpIngresoInicial.Enabled = False
            dtpIngresoFinal.Enabled = False
            Label11.Enabled = False
    ...
    Private Sub txtIDEmpleado_Leave(sender As Object, e As EventArgs) Handles txtIDEmpleado.Leave
        ...
        Con.Close()
        If txtEmpleado.Text = "" Then txtIDEmpleado.Clear()
    End Sub
        ...
        Try
            ...
            If chkEspecificarVenc.Checked = False Then
                If CDate(dtpIngresoInicial.Value) > CDate(dtpIngresoFinal.Value) Then
                    MessageBox.Show("La fecha inicial es mayor a la fecha final" & vbNewLine & vbNewLine & "Por favor revisar el rango de las fechas para procesar el reporte.", "Rango de Fechas no válidos", MessageBoxButtons.OK, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1)
                    Exit Sub
                End If
            End If
            ...
            Else
                crParameterDiscreteValue.Value = txtIDRegistro.Text
            End If
            crParameterFiledDefinitions = ObjRpt.DataDefinition.ParameterFields
            ...
            If txtIDDeduccion.Text = "" Then
                crParameterDiscreteValue.Value = 0
            Else
                crParameterDiscreteValue.Value = txtIDDeduccion.Text
            End If
            crParameterFiledDefinitions = ObjRpt.DataDefinition.ParameterFields
    ...
    Private Sub txtIDDeduccion_Leave(sender As Object, e As EventArgs) Handles txtIDDeduccion.Leave
        ConLibregco.Open()
        cmd = New MySqlCommand("Select Descripcion from Deducciones Where IDDeduccion='" + txtIDDeduccion.Text + "'", ConLibregco)
        txtDeduccion.Text = Convert.ToString(cmd.ExecuteScalar())
        ConLibregco.Close()
        If txtDeduccion.Text = "" Then txtIDDeduccion.Clear()
    End Sub

    Private Sub txtIDRegistro_Leave(sender As Object, e As EventArgs) Handles txtIDRegistro.Leave
        Con.Open()
        cmd = New MySqlCommand("Select SecondID from DeduccionesProcesadas Where IDeduccionesProcesadas='" + txtIDRegistro.Text + "'", Con)
        txtRegistroDeduccion.Text = Convert.ToString(cmd.ExecuteScalar())
        Con.Close()
        If txtRegistroDeduccion.Text = "" Then txtIDRegistro.Clear()
    End Sub
